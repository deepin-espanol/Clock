...
const char kDotFile[] = ":/src/external/timezone_dialog/images/indicator_active.png";
const char kTimezoneMapFile[] = ":/src/external/timezone_dialog/images/timezone_map_big@1x.svg";
...
const ZoneInfo TimezoneMap::getZoneinfo() const {
...
bool TimezoneMap::setTimezone(const QString &timezone)
{
	...
	const int index = GetZoneInfoByZone(total_zones_, QString(timezone).replace(" ", "_"));
    if (index > -1) {
        // 找到时区并标记到地图上
...
void TimezoneMap::popupZoneWindow(const QPoint& pos) {
  ...
  for (const ZoneInfo& zone : nearest_zones_) {
	zone_names.append(QString(GetLocalTimezoneName(zone.timezone, locale)).replace("_", " "));
  }
...
void TimezoneMap::remark() {
  ...
  if (!nearest_zones_.isEmpty()) {
	zone_pin_->setText(QString(GetLocalTimezoneName(current_zone_.timezone, locale)).replace("_", " "));
