...
#include "src/external/widgets/searchinput.h"
    ...
    , m_totalZones(installer::GetZoneInfoList())
{
	...
	connect(m_confirmBtn, &DSuggestButton::clicked, [this] {
		Q_EMIT confirmed(m_map->getTimezone());
		Q_EMIT confirmed(m_map->getZoneinfo());
    });
    ...
    QTimer::singleShot(0, [this] {
        ...
        for (const auto &zoneInfo : m_totalZones) {
			...
			completions << timezone.replace("_", " "); // timezone as completion candidate.
			...
			completions << localizedTimezone.replace("_", " ");
...
{
	m_map->setTimezone(QString(timezone).replace(" ", "_"));
}
...
{
	const QRect primaryRect = qApp->primaryScreen()->availableGeometry();
